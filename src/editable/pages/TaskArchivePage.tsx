...
const cardBase =
  'group block rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:border-[color:var(--tk-accent)]/50 hover:-translate-y-1'
    ...
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden border-b border-[var(--tk-line)]">
          <div className="pointer-events-none absolute inset-x-0 -top-40 h-[560px] bg-[radial-gradient(60%_60%_at_50%_0%,var(--tk-glow),transparent_70%)]" />
          <div className="relative mx-auto max-w-[var(--editable-container)] px-4 py-24 sm:px-8 sm:py-32 lg:px-20 lg:py-40">
            ...
            <EditableReveal index={1}>
              <h1 className="editable-display mt-8 max-w-4xl text-balance font-medium leading-[1.04] tracking-[-0.03em] text-[clamp(2.4rem,5vw,4.6rem)]">
                {voice?.headline || `Browse ${displayLabel}`}
              </h1>
            </EditableReveal>
            <EditableReveal index={2}>
              <p className="mt-7 max-w-2xl text-[17px] leading-8 text-[var(--tk-muted)]">
              ...
              <EditableReveal index={3}>
                <div className="mt-9 flex flex-wrap gap-2.5">
                  {voice.chips.map((chip) => (
                    <span key={chip} className="editable-mono rounded-full border border-[var(--tk-line)] bg-transparent px-3.5 py-1.5 text-[11px] uppercase tracking-[0.18em] text-[var(--tk-muted)]">
                      / {chip}
                    </span>
                  ))}
                </div>
            ...
            <EditableReveal index={4}>
              <div className="mt-14 flex flex-col gap-4 border-t border-[var(--tk-line)] pt-8 sm:flex-row sm:items-center sm:justify-between">
                <p className="editable-mono text-[13px] text-[var(--tk-muted)]">
                  <span className="text-[var(--tk-text)]">{posts.length}</span> · {categoryLabel}
                </p>
                <form action={basePath} className="flex items-center gap-2.5">
                  <div className="relative">
                    <select
                      ...
                      className="editable-mono h-11 appearance-none rounded-full border border-[var(--tk-line)] bg-[var(--tk-surface)] pl-4 pr-10 text-[13px] font-medium text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
                  ...
                  <button className="inline-flex h-11 items-center rounded-full bg-[var(--tk-accent)] px-5 text-sm font-medium text-[var(--tk-on-accent)] transition duration-500 hover:opacity-90">
                    Apply
                  </button>
                </form>
              </div>
        ...
        {task === 'pdf' ? (
          <div className="mx-auto w-full max-w-[var(--editable-container)] px-4 pt-10 sm:px-8 lg:px-20">
            <Ads slot="header" size={pickRandom(getSlotSizes('header'))} showLabel className="mx-auto w-full" />
          </div>
        ) : null}

        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-20 sm:px-8 sm:py-24 lg:px-20 lg:py-32">
          {posts.length ? (
            <>
            ...
            <div className="mx-auto max-w-xl rounded-3xl border border-dashed border-[var(--tk-line)] bg-[var(--tk-surface)] px-8 py-20 text-center">
              ...
              <p className="mt-3 text-[15px] leading-7 text-[var(--tk-muted)]">Try another category, or check back after new entries are published.</p>
            </div>
          )}
          ...
          {posts.length ? (
            <nav className="mt-20 flex items-center justify-center gap-3 text-sm">
              {pagination.hasPrevPage ? (
                <Link href={pageHref(basePath, category, page - 1)} className="editable-mono rounded-full border border-[var(--tk-line)] px-5 py-2.5 text-[13px] font-medium uppercase tracking-[0.14em] transition hover:border-[var(--tk-accent)] hover:text-[var(--tk-accent)]">
                  Previous
                </Link>
              ) : null}
              <span className="editable-mono rounded-full border border-[var(--tk-line)] bg-[var(--tk-surface)] px-5 py-2.5 text-[13px] font-medium uppercase tracking-[0.14em] text-[var(--tk-muted)]">
  ...
  return (
    <Link href={href} className={`${cardBase} overflow-hidden`}>
      ...
      <div className="p-7 sm:p-8">
        <div className="editable-mono flex items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-[var(--tk-accent)]">
          <span>/ {category}</span>
          <span className="text-[var(--tk-muted)]">· No. {String(index + 1).padStart(2, '0')}</span>
        </div>
        <h2 className="editable-display mt-5 text-2xl font-medium leading-snug tracking-[-0.02em]">{post.title}</h2>
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex items-center gap-6 p-6 sm:p-7`}>
      <div className="flex h-28 w-28 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-raised)]">
        {logo ? <img src={logo} alt="" className="h-full w-full object-cover" /> : <BriefcaseBusiness className="h-9 w-9 text-[var(--tk-muted)]" />}
      </div>
      <div className="min-w-0 flex-1">
        <p className="editable-mono text-[11px] uppercase tracking-[0.22em] text-[var(--tk-accent)]">/ Directory entry</p>
        <h2 className="editable-display mt-2 truncate text-xl font-medium tracking-[-0.02em]">{post.title}</h2>
        ...
        <div className="mt-4 flex flex-wrap gap-3 text-xs text-[var(--tk-muted)]">
          ...
          {website ? <span className="inline-flex items-center gap-1.5"><Globe className="h-3.5 w-3.5 text-[var(--tk-accent)]" /> Site</span> : null}
        </div>
      </div>
      <ArrowUpRight className="h-5 w-5 shrink-0 text-[var(--tk-muted)] transition duration-500 group-hover:text-[var(--tk-accent)]" />
    </Link>
  )
}
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-7 sm:p-8`}>
      <div className="flex items-start justify-between gap-4">
        ...
        {condition ? <span className="editable-mono rounded-full bg-[var(--tk-accent-soft)] px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-[var(--tk-accent)]">{condition}</span> : null}
      </div>
      <h2 className="editable-display mt-6 text-xl font-medium leading-snug tracking-[-0.02em]">{post.title}</h2>
      <p className="mt-4 line-clamp-3 flex-1 text-[14px] leading-7 text-[var(--tk-muted)]">{getSummary(post)}</p>
      <div className="mt-7 flex items-center justify-between border-t border-[var(--tk-line)] pt-4 editable-mono text-[11px] uppercase tracking-[0.16em] text-[var(--tk-muted)]">
        <span>{location ? `/ ${location}` : '/ Details inside'}</span>
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-6 block break-inside-avoid overflow-hidden rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1">
      <div className={`relative overflow-hidden ${index % 3 === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
        <img src={image} alt="" className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.04]" />
        <div className="absolute inset-0 bg-[linear-gradient(180deg,transparent_45%,rgba(13,13,13,0.85))]" />
        <div className="absolute inset-x-0 bottom-0 p-6">
          ...
          <span className="editable-mono mt-2 inline-flex items-center gap-1.5 text-[11px] uppercase tracking-[0.18em] text-white/75">
...
function BookmarkArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex gap-5 p-7`}>
      ...
      <div className="min-w-0 flex-1">
        <span className="editable-mono text-[11px] uppercase tracking-[0.2em] text-[var(--tk-muted)]">/ Signal · {String(index + 1).padStart(2, '0')}</span>
        ...
        {website ? <p className="editable-mono mt-3 truncate text-xs text-[var(--tk-accent)]">{cleanDomain(website)}</p> : null}
      </div>
    </Link>
  )
}
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col p-7 sm:p-8`}>
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="editable-mono rounded-full border border-[var(--tk-line)] px-3 py-1 text-[11px] uppercase tracking-[0.16em] text-[var(--tk-muted)]">/ {category}</span>
      </div>
      <h2 className="editable-display mt-7 text-xl font-medium leading-snug tracking-[-0.02em]">{post.title}</h2>
      <p className="mt-4 line-clamp-3 flex-1 text-[14px] leading-7 text-[var(--tk-muted)]">{getSummary(post)}</p>
      <span className="mt-7 inline-flex items-center gap-1.5 text-sm font-medium text-[var(--tk-accent)]">
...
function ProfileArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col items-center p-8 text-center`}>
      ...
      {role ? <p className="editable-mono mt-2 text-[11px] uppercase tracking-[0.16em] text-[var(--tk-accent)]">/ {role}</p> : null}
